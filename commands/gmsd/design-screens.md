...
agent: gmsd-ui-specifier (for complex specs), gmsd-ui-researcher (for pattern research)
---
...
@~/.claude/get-more-shit-done/ui-design/references/design-systems.md
...
1. **Check for UI-CONTEXT.md:**
   - If missing, suggest running `/gmsd:init` first
...
Last updated: [date]
Generated by: /gmsd:design-screens
...
Pending: Run `/gmsd:define-components` for full inventory
...
Document patterns in `.planning/design/UI-PATTERNS.md` or note for `/gmsd:patterns` command.
</step>
...
`/gmsd:export stitch`    Visual designs
`/gmsd:export v0`        React components
