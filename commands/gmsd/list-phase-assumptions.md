...
In yolo mode, auto-save (skip asking, proceed directly to Step 5).

### Step 4b: Ask About Saving (guided and balanced modes only)

After the conversational review is complete and the user has provided feedback, end with:
...
After the user responds and any corrections are discussed, ask:

```
Would you like me to save these assumptions and corrections?
1. **Yes** -- Write ASSUMPTIONS.md and feed corrections into CONTEXT.md
2. **No** -- Keep this conversational (no files created)
```

- If **Yes**: proceed to Step 5 (save files)
- If **No**: skip Steps 5 and 6, proceed directly to Step 7 (update state with a note that assumptions were reviewed conversationally)
- In **yolo mode**: always auto-save (skip this question)

### Step 5: Save ASSUMPTIONS.md (only if user said yes or yolo mode)
...
### Step 6: Feed Corrections into CONTEXT.md (only if user said yes or yolo mode)
...
If conversational only (user declined to save):
```json
{
  "command": "/gmsd:list-phase-assumptions {N}",
  "timestamp": "{ISO timestamp}",
  "result": "Surfaced {count} assumptions ({confirmed} confirmed, {corrected} corrected, {removed} removed). Reviewed conversationally (no files saved)."
}
```
...
Current: Phase {N} -- {phase_name} | Assumptions: reviewed{" (saved)" if files written, "" if conversational} | Mode: {mode}
