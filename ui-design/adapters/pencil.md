...
```bash
/gmsd:export pencil           # All screens
/gmsd:export pencil SCR-01    # Single screen
/gmsd:export pencil SCR-01,SCR-02  # Multiple screens
```

### Automatic Workflow

When `/gmsd:export pencil` is invoked:
...
4. **Record in system**
   ```bash
   /gmsd:realize SCR-01 --source pencil
   ```

5. **Export to code** (if needed)
   ```bash
   /gmsd:export v0 SCR-01  # Generate V0 prompts from same spec
   ```

6. **Sync check**
   ```bash
   /gmsd:ui-sync  # Compare spec to Pencil design
   ```
