...
1. **Figma Export Path (Recommended)**
   ...
   - Use `/gmsd:realize --import figma [file-url]` to analyze
...
2. **Code Export Path**
   ...
   - Use `/gmsd:import-design --source stitch --files [paths]`
...
4. **Record in system:**
   - Run `/gmsd:realize` to track the generation
   - Update UI-REGISTRY.md
   - Note any deviations

5. **Sync back:**
   - If modifications were made in Figma, run `/gmsd:ui-sync`
