...
2. **Track in UI System**
   ```bash
   /gmsd:import-design --source v0 --files src/components/login-form.tsx
   ```
...
5. **Sync Check**
   - Run `/gmsd:ui-sync` to compare spec vs implementation
...
4. **Register in UI System**
   ```bash
   /gmsd:import-design --source v0 --files [paths]
   ```
